(() => {
  ...
  const title = document.querySelector('#certTitle');
  const body = document.querySelector('#certBody');
  const meta = document.querySelector('#certMeta');
  ...
  const certificateData = {
    google: { title: 'AI-Powered Performance Ads Certification', body: 'This acknowledges that Deepak Kumar has successfully completed and is certified in AI-Powered Performance Ads Certification.', meta: 'Google · Issue Date: August 10, 2026 · Expiry: August 10, 2027 · Certificate ID: 191126599' },
    analytics: { title: 'Introduction to Data Analytics', body: 'Certificate awarded to Deepak Kumar for Introduction to Data Analytics.', meta: 'Simplilearn · 22 October 2025 · Certificate code: 9212089' }
  };
  document.querySelectorAll('.cert-link').forEach(button => button.addEventListener('click', () => {
    const data = certificateData[button.dataset.cert];
    if (!data || !modal) return;
    title.textContent = data.title; body.textContent = data.body; meta.textContent = data.meta;
